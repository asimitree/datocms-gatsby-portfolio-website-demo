    ...
    url: {
      ...
      resolve: function resolve(node, args) {
        ...
        if (args.imgixParams && Object.keys(args.imgixParams).length > 0) {
          var query = _objectSpread({}, args.imgixParams);

          if (node.focalPoint && query.fit === 'crop' && (query.h || query.height) && (query.w || query.width) && (!query.crop || query.crop === 'focalpoint') && !query['fp-x'] && !query['fp-y']) {
            query.crop = 'focalpoint';
            query['fp-x'] = node.focalPoint.x;
            query['fp-y'] = node.focalPoint.y;
          }

          url += "?".concat(queryString.stringify(query));
        }
